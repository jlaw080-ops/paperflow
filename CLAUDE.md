...
- CRITICAL: `docs/PRD.md`의 **MVP 제외 사항**에 있는 기능을 임의로 만들지 말 것(양방향 동기화, 협업/댓글/버전관리, 검색, 수식 등). 필요해 보이면 먼저 사람에게 물을 것.
  ...
  - 범위 변경 이력: HTML 파일 가져오기·렌더링은 사용자 승인으로 MVP 범위에 편입됨(구현 완료). `.html`/`.htm`을 import하면 `documents.format='html'`로 저장하고, 공개 보기·미리보기에서 DOMPurify sanitize 후 sandbox iframe(`HtmlView`)으로 렌더한다. 이는 "표준 마크다운만 렌더" 규칙의 명시적 예외이며, raw HTML을 푸는 대신 sanitize+iframe 격리로 XSS를 방어한다.
